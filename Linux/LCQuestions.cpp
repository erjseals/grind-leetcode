...
std::vector<std::string> LCQuestions::letterCombinations(std::string digits) {
  std::vector<std::string> t9 = {"",    "",    "abc",  "def", "ghi",
                                 "jkl", "mno", "pqrs", "tuv", "wxyz"};

  // Initialize the result std::vector with an empty std::string for the base
  // case swap
  ...
  for (unsigned int i = 0; i < digits.size(); i++) {
    ...
    // Only digits 2 to 9 are valid, but I handle 0 and 1 incase user hits those
    // numbers
    if (0 > digit || digit > 9)
      continue;
...
void LCQuestions::genParenHelper(std::vector<std::string> &vec, std::string str,
                                 int l, int r) {
  if (l == 0 && r == 0) {
    vec.push_back(str);
    return;
